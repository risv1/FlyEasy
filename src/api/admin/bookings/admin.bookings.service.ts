...
export class AdminBookingService {
  ...
  async getBookingById(bookingId: string) {
    const [booking]: BookingDto[] = await db
      .select()
      .from(bookings)
  ...
  async updateBooking(bookingId: string, booking: any) {
    const [bookingExists]: BookingDto[] = await db
      .select()
      .from(bookings)
      .where(eq(bookings.id, bookingId));
    if (!bookingExists) {
    ...
    const updatedBookingData = {
      flightId: booking.flightId
        ...
        : bookingExists.flightId,
      passengerId: booking.userId
        ? String(booking.passengerId)
        : bookingExists.passengerId,
      passengerName: booking.passengerName
        ? String(booking.passengerName)
        : bookingExists.passengerName,
      passengerEmail: booking.passengerEmail
        ? String(booking.passengerEmail)
        : bookingExists.passengerEmail,
