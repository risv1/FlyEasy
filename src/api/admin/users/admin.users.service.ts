...
export class AdminUserService {
    ...
    async getUserById(userId: string) {
        const [user]: UserDto[] = await db.select().from(users).where(eq(users.id, userId));
        if(!user) {
    ...
    async updateUser(userId: string, user: UserDto) {
        const [userExists]: UserDto[] = await db.select().from(users).where(eq(users.id, userId));
        if(!userExists) {
            throw new HttpException('User not found', 404);
        }
        ...
        const updatedUser = await db.update(users).set(updatedUserData).where(eq(users.id, userId));
        if(!updatedUser) {
